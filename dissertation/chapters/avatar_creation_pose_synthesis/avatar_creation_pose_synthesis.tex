...
The animation speedupe compared to the previous synthesizer for some \gls{lsf} signs are shown in table~\ref{tab:faster_executions}.

\begin{table}
    \centering
    \begin{tabular}{|c|c|}
        \hline
        \textbf{Interface} & \textbf{Speedup} \\
        \hline
        Sign & Speedup \\
        \emph{arbre} (tree) & \FPeval{\result}{1.58/0.63} \\
        \emph{bien} (good) & \FPeval{\result}{2.93/1.03} \\
        \emph{hier} (yesterday) & \FPeval{\result}{5.8/2.2} \\
        \emph{armoire} (cupboard) & \FPeval{\result}{21.85/8.6} \\
        \hline
    \end{tabular}
    \caption{Speedup compared to the previous synthesizor}
...
The configuration for the posture optimization algorithm was as follows:
\begin{itemize}
    \item Iterations: 20
