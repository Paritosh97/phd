...
The template was created by extracting bone rotations as quaternions using template SMPL-X reconstructions of the avatar~\cite{Forte_2023_CVPR} and then converting them into motion curves (figure~\ref{fig:motion_curves_smplx}). The final blended rotation can be calculated using \gls{slerp} between the \( n \) quaternions \( q_1, q_2, \dots, q_n \):
...
Figure~\ref{fig:motion_curves_mocap} similar template creation but using AZee annotated motion capture~\cite{bertin2022rosetta} retargeted on our avatar.
...
\begin{figure}
    ...
    \hline
    TODO & \includegraphics[width=1.5in]{chapters/intermediate_blocks_pose_correction/images/standard_interpolation_todo1.png} & \includegraphics[width=1.5in]{chapters/intermediate_blocks_pose_correction/images/template_interpolation_todo1.png} \\
    \hline
