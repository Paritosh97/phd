...
\begin{itemize}
  \item \textbf{Synced Score}: A recursive structure containing other scores with some sync rules (relative timing information of the underlying scores) (figure~\ref{fig:azee_score_example}).
...
\begin{itemize}
  ...
  \item \textbf{Morph}: Constrains a morphological change of a body part(s). However, it only defines the syntax (with no actual specification more explained later in section~\ref{ch:avatar_creation_pose_synthesis:proc_rig_signing_avatars:morph_constraints:skel_morphs} of chapter~\ref{ch:avatar_creation_pose_synthesis})
...
Lastly, an AZee description can also be drawn in the form of \gls{svg} images using \gls{azvd}~\cite{filhol2024software}. Figure~\ref{fig:azvd} shows the AZee verbalizing diagram for the \gls{utterance} "a person aged 52".
...
A rig is the final system used by the artists to animate the avatars. This is done by implementing all the above systems in order, i.e., creating a skeleton, mesh, texture, weight painting, facial blendshapes. The skeleton is often modeled in separate layers of bones (with certain constraints) for different tasks such as \gls{ik}, \gls{fk} etc. Figure~\ref{fig:rig_example} shows an example of a rigged character, the "Rain rig" by Blender Studio.
...
\paragraph{Skeletal Blendshapes}
\label{ch:background_work:sign_language_synthesis:3d_techniques:avatar_animation:skeletal_blendshapes}

Blendshapes (or Morph Targets) are a technique for animating 3D meshes by smoothly transitioning between predefined shapes, often used for facial expressions. Skeletal blendshapes extend this by applying shape blending to skeletal structures, allowing for deformations that react to bone movement, enhancing realism in animations like muscle flexing or limb poses. Figure~\ref{fig:skeletal_blendshape} shows skeletal blendshape for the fist.

\begin{figure} 
  \centering \includegraphics[width = 2.5in]{chapters/background_work/images/skeletal_blendshapes.jpg} 
  \caption{Skeletal blendshape for fist close and open} 
  \label{fig:skeletal_blendshape} 
\end{figure}
...
\begin{figure}
  \centering \includegraphics[width = 5in]{chapters/background_work/images/ccdik.png}
  \caption{Cyclic Coordinate Descent (CCD) IK solving (changes the rotation of a joint, one at a time)}
  \label{fig:ccdik}
\end{figure}

\begin{figure}
  \centering \includegraphics[width = 5in]{chapters/background_work/images/fabrik.png}
  \caption{FABRIK solving (updates coordinates in two passes)}
  \label{fig:fabrik}
\end{figure}


These classical \gls{ik} techniques, while computationally efficient, are prone to singularities, have limited support for complex joint constraints, and often result in unnatural or biomechanically unrealistic poses. 

Figure~\ref{fig:problems_classical} demonstrates five such examples:
\begin{itemize}
    \item \textbf{Multiple solutions:} When the end effector results in multiple configurations of the chain.
    \item \textbf{Singularity:} Occurs when the Jacobian matrix loses rank, making it impossible to control the end effector’s motion in certain directions. This results in the loss of degrees of freedom.
    \item \textbf{Redundancy:} When there are more degrees of freedom in the system than required to achieve the desired end effector position.
    \item \textbf{Joint limits:} When the joints of the chain reach their limits, preventing the arm from reaching certain positions or orientations.
    \item \textbf{Instability:} When the system approaches a singularity, leading to highly sensitive behavior in joint movements and instability in motion control.
\end{itemize}

\begin{figure}
  \centering \includegraphics[width = 5in]{chapters/background_work/images/problems_classical.png}
...
Although AZee is multi-track by nature (where the linguist has the ability to create the track), the low-level AZee synthesizer uses an \emph{Animated Score} which is a flattened version of an AZee Synced Score, where all the tracks are merged into a single track. This flattening is done by collecting all the \gls{posture_constraint}s for each frame and applying them to the corresponding bone rotations during animation. Figure~\ref{fig:azee_score_armoire_combined} visualizes this flattening process showing the flattened version of the AZee Synced Score.
...
\subparagraph{Bone hierarchy}
\label{ch:background_work:sign_language_synthesis:3d_techniques:sign_language_synthesis_systems:azee_based:low_level_synthesizer_for_azee:bone_hierarchy}
...
During evaluation, the low-level synthesizer creates postures for each of the frames using this Animated Score. For this, they used the algorithm~\ref{alg:trimmed_constraint_based_optimization} for the pose optimization for each of the constraints to generate the postures. 
...
Although this approach provides better coverage using AZee descriptions by directly generating animations from the linguist's input without relying on artists, it is not extensible for use with pre-animated motion data blocks for larger sequences. This is because the flattened representation does not retain the original reference to the corresponding animation block on the timeline. More importantly, it generates robotic animations since it does not consider the nuances of natural human motion. This work therefore is the other side of the coin to a high-level synthesis technique like Paula, which are hindered by the labor-intensive nature of hand-crafted animations.
