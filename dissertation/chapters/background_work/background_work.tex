...
The AZee interpreter parses this code recursively, generating the corresponding(figure \ref{fig:timeline_example}) multi-track timeline for the avatar to animate.

\begin{figure}
  \centering \includegraphics[width = 2.5in]{chapters/background_work/images/timeline_example.png}
  \caption{AZee visual description for a person aged 52}
  \label{fig:timeline_example}
\end{figure}

This timeline is geenrated using the recursive algorithm \ref{alg:azee_recursion}.

todo
\begin{algorithm}
  \caption{AZee Recursion}
  \label{alg:azee_recursion}
  \begin{algorithmic}
    \Function{AZee}{$code$}
      \If{$code$ is a leaf node}
        \State \Return $code$
      \EndIf
      \State $result \gets$ empty list
      \For{$child$ in $code$}
        \State $result \gets$ \Call{AZee}{$child$}
      \EndFor
      \State \Return $result$
    \EndFunction
  \end{algorithmic}
...
The step by step recursion can be seen in {annex:azee_recursion}.
