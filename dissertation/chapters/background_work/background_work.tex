...
% todo add azee is best representation of SL and cite Camille
% thus any other system are not that good 
...
% todo fix definition - they help in idetifying patterns etc
...
The synthesizer uses an armature contained 57 bones (appendix~\ref{old_bone_list}). These bones were structured to accommodate the unique skeletal system requirements of the AZee framework, which utilizes a "rotation first" paradigm. However, since most 3D softwares (including blender) operate on a "translation first" paradigm, a conversion process was necessary. This conversion involved mapping each AZee bone to two corresponding armature bones: one for translation and one for rotation, with the rotation bone named by appending a ".rot" suffix to the original bone name (figure~\ref{fig:old_bone_structure}).
...
%todo acknowledge johns work its good for shortcuts etc and also uses azee a bit
%todo todo acknowledge otehr work as well
%todo acknowledge azee as well as it is a good source of input
%todo acknowledge opensource stuff
%acknowledge fabrizio as well as it is good and he used azee BUT he flattened.
%take parts of everything and link to my contribution
\begin{itemize} 
  \item Linear systems for \gls{sl} representation are limited in their expressive capacity and fail to capture the complexity of natural \gls{sl}. %todo therefore we will not use them
