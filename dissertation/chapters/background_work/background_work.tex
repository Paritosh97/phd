...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Function{create\_blocks}{name, score, parent\_name, parent\_score, dynamic\_points}
      \If{parent\_score is not None}
        \State Modify name and sync rules for recursion
      \EndIf
      \State Update block timings
      \If{score has dynamic\_point}
        \State Append dynamic\_point to dynamic\_points
      \EndIf
      \If{shortcut\_action exists and use\_shortcut\_actions is true}
        \State Increment sync order
        \State Create and append preanimated block
        \State Update score timings
        \State Update parent sync rule if needed
        \State \Return block
      \EndIf
      \If{score is SyncedScore}
        \ForAll{child\_score in score.block\_contents}
          \State Create and append meta block
          \State Recursively create composite blocks
        \EndFor
      \ElsIf{score is CstrScore or HoldScore}
        \State Increment sync order
        \State Create and append block based on type
      \EndIf
      \If{parent\_score is not None}
        \State Update parent score timings
      \EndIf
      \State \Return block
    \EndFunction
  \end{algorithmic}
